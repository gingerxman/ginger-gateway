...
class Watcher:  
	...
	def watch(self):  
		while True:
			import subprocess
			proc = subprocess.Popen("./ginger-gateway", stdout=subprocess.PIPE)
			with open('_pid', 'wb') as f:
			...
			try:
				while True:
					...
					if line != '':
						...
						print '\n\n========= Compile EEL ========='
						os.system('go build -o ginger-gateway -mod=vendor -v ./main.go')
						break
...
if __name__ == '__main__':
	exit_status = os.system('go build -o ginger-gateway -mod=vendor -v ./main.go')
	if exit_status != 0:
